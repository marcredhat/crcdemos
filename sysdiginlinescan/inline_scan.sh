...
Sysdig Inline Scanner/Analyzer --
  ...
  Wrapper script for performing vulnerability scan or image analysis on local podman images, utilizing the Sysdig inline_scan container.
...
Sysdig Inline Analyzer --
  ...
  Script for performing analysis on local podman images, utilizing the Sysdig analyzer subsystem.
  ...
  Images should be built & tagged locally.
    ...
    Usage: ${0##*/} analyze -s <SYSDIG_REMOTE_URL> -k <API Token> [ OPTIONS ] <FULL_IMAGE_TAG>
      ...
      -P  [optional] Pull podman image from registry
...
get_and_validate_images() {
    ...
    if [[ "${#FAILED_IMAGES[@]}" -gt 0 ]]; then
        ...
        if [[ "${#FAILED_IMAGES[@]}" -ge "${#IMAGE_NAMES[@]}" ]]; then
            printf '\n\t%s\n\n' "ERROR - no local podman images specified in script input: ${0##*/} ${IMAGE_NAMES[*]}" >&2
            display_usage >&2
            exit 1
        fi
...
prepare_inline_container() {
    ...
    if [[ -z "${SYSDIG_CI_IMAGE-podman.io/anchore/inline-scan:v0.5.0}" ]]; then
        printf '\n%s\n' "Pulling ${INLINE_SCAN_IMAGE}"
        podman pull "${INLINE_SCAN_IMAGE}"
    else
        printf '\n%s\n' "Using local image for scanning -- ${INLINE_SCAN_IMAGE}"
    fi

    # setup command arrays to eval & run after adding all required options
    CREATE_CMD=('podman create --name "${DOCKER_NAME}"')
    RUN_CMD=('podman run -i --name "${DOCKER_NAME}"')
...
start_analysis() {
    ...
    if [[ ! "${i_flag-""}" ]]; then
        for i in "${SCAN_IMAGES[@]}"; do
            ...
            IMAGE_ID=$(podman image inspect "$i" -f "{{.Id}}" | cut -f2 -d ":" )
    ...
    if [[ "${m_flag-""}" ]]; then
        ...
        COPY_CMDS+=('podman cp "${MANIFEST_FILE}" "${DOCKER_NAME}:/anchore-engine/$(basename ${MANIFEST_FILE})";')
    fi
    if [[ "${f_flag-""}" ]]; then
        CREATE_CMD+=('-f "${DOCKERFILE}"')
        COPY_CMDS+=('podman cp "${DOCKERFILE}" "${DOCKER_NAME}:/anchore-engine/$(basename ${DOCKERFILE})";')
    fi
    ...
    podman cp "${DOCKER_NAME}:/anchore-engine/image-analysis-archive.tgz" "/tmp/sysdig/${analysis_archive_name}"
...
save_and_copy_images() {
    ...
    for image in "${SCAN_IMAGES[@]-}"; do
        ...
        # If image is passed without a tag, append :latest to podman save to prevent skopeo manifest error
        if [[ ! "${image}" =~ [:]+ ]]; then
            podman save "${image}:latest" -o "${save_file_path}"
        else
            podman save "${image}" -o "${save_file_path}"
        fi
        ...
        if [[ -f "${save_file_path}" ]]; then
            ...
            printf '\n\t%s\n\n' "ERROR - unable to save podman image to ${save_file_path}." >&2
            display_usage >&2
            exit 1
        fi

        if [[ ! "${v_flag-""}" ]]; then
            podman cp "${save_file_path}" "${DOCKER_NAME}:/anchore-engine/${save_file_name}"
            rm -f "${save_file_path}"
        fi
    done
}
...
cleanup() {
    ...
    for id in ${DOCKER_ID}; do
        ...
        while (podman ps -a | grep "${id:0:10}") > /dev/null && [[ "${timeout}" -lt 12 ]]; do
            ...
            printf '\n%s\n' "Cleaning up podman container: ${id}"
            ((timeout=timeout+1))
            sleep 5
        done
