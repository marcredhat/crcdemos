...
== Install Change Data Capture solution (Debezium)
...
[marc@dell-r730-019 plugins]$ oc exec -it my-cluster-kafka-0 -- /opt/kafka/bin/kafka-topics.sh --list --bootstrap-server localhost:9092
...
----
  ...
  [marc@dell-r730-019 plugins]$ oc exec -it my-cluster-kafka-0 -- /opt/kafka/bin/kafka-console-consumer.sh     --bootstrap-server my-cluster-kafka-bootstrap:9092     --from-beginning     --property print.key=true     --topic dbserver1.inventory.customers | grep m@redhat.com
