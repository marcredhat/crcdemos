...
== Draft lab guide showing how to set up CodeReady Containers, OpenShift 4.1, Ansible Agnostic Deployer and OpenDataHub on a RHEL 8.1 physical server
...
[marc@dell-r730-019 kie-cloud-operator]$ more  deploy/catalog_resources/redhat/1.2.0/businessautomation-operator.1.2.0.clusterserviceversion.yaml
...
[marc@dell-r730-019 kie-cloud-operator]$ oc create -f deploy/catalog_resources/redhat/1.2.0/businessautomation-operator.1.2.0.clusterserviceversion.yaml

----
clusterserviceversion.operators.coreos.com/businessautomation-operator.1.2.0 created
----

[marc@dell-r730-019 ~]$ eval $(crc oc-env) && oc login -u kubeadmin -p <password> https://api.crc.testing:6443
Login successful.
...
Add console-cr-form-openshift-marketplace.apps-crc.testing to /etc/hosts
...
Open the OpenShift console at https://{openshift_host}:8443/console and open the “rhpam7-trial” project. Go the “Overview” page and wait until both the Business Central and Process Server pods are up and running.

Go to “Applications -> Routes”.  Click on the “Hostname” of “rhpam7-trial-rhpamcentr”, which should open the Business Central login screen.

On your laptop, sudo ssh marc@dell-r730-019.dsal.lab.eng.rdu2.redhat.com -L 8888:rhpam7-rhpamcentr-rhpam7-trial.apps-crc.testing:80
...
See image:images/redhatprocessautomation1[title="Red Hat Process Automation Manager - Marc demo"] 
